...
impl Icosphere {
    ...
    fn subdivide_if_nessairy_and_clamp_level(&mut self, mut level: usize) -> usize {
        ...
        for _ in (self.faces.len())..level + 1 {
    ...
    fn subdivide_icosphere(&mut self) {
        ...
        let mut create_new_vertex_on_edge_center = |i_a: usize, i_b: usize| -> usize {
            // Edges are independent of vertex order,
            // thus we sort the vertecies in order to use it as key
            let a = self.vertecies[i_a];
            let b = self.vertecies[i_b];
            ...
            let new_vertex_index = self.vertecies.len();
